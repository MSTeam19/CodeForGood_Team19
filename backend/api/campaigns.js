...
router.get('/:id/leaderboard', async (req, res) => {
  try {
    ...
    // Build query for leaderboard_region materialized view
    let query = supabase
      .from('leaderboard_region')
      .select('*');

    // Filter by campaign if needed (once we have campaign relationship)
    ...
    // Order by total amount descending for leaderboard
    ...
    const response = {
      ...
      regions: data.map(region => ({
        ...
        totalAmountCents: region.total_amount_cents,
        donationCount: region.donation_count,
        highestSingleDonationCents: region.highest_single_donation_cents,
